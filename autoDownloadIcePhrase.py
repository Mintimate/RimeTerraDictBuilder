...
def download_phrase_with_request(source_save_path, target_save_path,
                                 download_url, target_file_name):
...
def __rename_dict_to_mint_style(source_path, target_path, file_name):
    ...
    with open(source_path, 'r', encoding='utf-8') as file:
        content = file.read()
    # 替换name中的原本文件名
    content = re.sub(r'#@/db_name(\s*)(\S+)', f'#@/db_name  {file_name}', content)
...
if __name__ == '__main__':
    ...
    DOWNLOAD_ITEM = USER_CONFIG['rime_ice_dict']['phrase_source_path']
